...
<script type="text/javascript">
	...
	function getAjax(n){
		if(pageNumber<total && n==1)
			...
			return false;
		var name = $("#search").val();
		$.getJSON("/ShiZunSystem/showDept",{"pageSize":pageSize,"pageNumber":pageNumber,"name":name},function(pageInfo){
			var dept = pageInfo.list;
			var result = "";
			var pageNum = "";
			total = pageInfo.total==0?1:pageInfo.total;
			for(var i=0;i<dept.length;i++){
				result += "<tr>";
				result += "<td><input name='del' type='checkbox' value=" + dept[i].id + " /></td>";
				result += "<td>" + dept[i].no + "</td>";
				result += "<td>" + dept[i].name + "</td>";
				result += "<td>" + dept[i].sname + "</td>";
				result += "<td>" + dept[i].remark + "</td>";
				result += "<td width='8%' class='mian_b_icon_01'><a href='/ShiZunSystem/editDeptById?id=" + dept[i].id + "'>编辑</a></td>";
				result += "<td width='8%' class='mian_b_icon_02'><a href='/ShiZunSystem/showDeptById?id=" + dept[i].id + "'>查看</a></td>";
				result += "</tr>";
			}
			$(".tbody").html(result);
...
<body class="mian_bj">
	<div class="mian_top_01">
		...
		<div class="mian_b">
			<div class="mian_b1">
				<a href="AddBranch" title="添加"><p class="mian_b1_a1"></p></a>
				<a href="javascript:void(0)" title="删除" onclick="delDept()"><p class="mian_b1_a2"></p></a>
			...
			<form action="/ShiZunSystem/delDept" method="post">
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mian_b_bg">
					<tr>
						<td width="3%" class="mian_b_bg_lm"><input type="checkbox"/></td>
